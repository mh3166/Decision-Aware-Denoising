...
def OS_MS_VGC_sep(T, L, nu, h, n_b, B, B0, S, sol, obj):
    ...
    for s in range(S):
        ...
        # delta_h_os = delta_h * ((h + 2)/(nu * h + 2 * nu ** 0.5)) ** 0.5
        delta_h_os_1 = delta_h[B0 : n_b - B0].copy().reshape(-1, B)
        # OS VGC procedure
        sol_os_h, obj_os_h = solve_problem(T + delta_h_os_1)
        total_os_obj += (obj_os_h - obj)/h
        # MS VGC procedure
        ...
        for i in range(B+2):
            ...
            ms_vgc_i = np.sum((V_h - V)/h)
            total_ms_obj += ms_vgc_i
    ms_vgc = total_ms_obj/S
    os_vgc = total_os_obj/S
    
    return ms_vgc, os_vgc
...
total_trials = 200
n_arr = [20, 40, 80, 160, 320, 640]
h_arr = [2 ** (-6), 2 ** (-5), 2 ** (-4), 2 ** (-3), 2 ** (-2), 2 ** (-1), 1, 1.5]
B_arr = [4, 20]
...
exp_array = []
for n in n_arr:
    for h_ind, h in enumerate(h_arr):
        for B in B_arr:
            exp_array.append([n, h, B, h_ind])

summary_table = [['n', 'h', 'h_ind', 'B', 't', 'in_samp_perf', 'oos_perf', 'est', 'method']]
...
for exp in exp_array:
    ...
    #One-Shot and Multi-Shot VGC
    ms_vgc, os_vgc = OS_MS_VGC_sep(plug_in, L, nu, h, n_b, B, B0, S, sol, obj)
    ...
    row_ms = [n, h, h_ind, B, t, insamp_perf, oos_perf, ms_vgc_est, 'MS']
    row_os = [n, h, h_ind, B, t, insamp_perf, oos_perf, os_vgc_est, 'OS']
    row_sc = [n, h, h_ind, B, t, insamp_perf, oos_perf, stein_corr_est, 'Stein']
...
summary_df_2.to_csv('data_syn/os_vs_ms_sep_n_' + str(trial) + '.csv', index = False)